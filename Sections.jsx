      ...
      <div style={{ position:"relative", zIndex:2, margin:"0 auto",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:36 }}>
          ...
          <span style={{ fontFamily:"var(--font-mono)", fontSize:11,
            ...
            AI systems for legacy industries · Jupiter, FL
          </span>
        </div>

        {/* Split display lockup — Roan in cream, Co. in magenta italic */}
        <h1 style={{ fontFamily:"var(--font-serif)", fontWeight:400,
          ...
          WebkitTextFillColor:"var(--cream)" }}>Roan</h1>
        <h1 style={{ fontFamily:"var(--font-serif)", fontWeight:400, fontStyle:"italic",
          fontSize:"clamp(4rem,11.5vw,11rem)", lineHeight:.86,
          letterSpacing:"-.025em", margin:0, color:"var(--accent)",
          WebkitTextFillColor:"var(--accent)" }}>Co.</h1>

        {/* Subhead */}
        <p style={{ marginTop:40, maxWidth:560, fontSize:18, lineHeight:1.6,
          color:"rgba(245,239,224,.78)" }}>
          We ship AI systems for industries that still run on spreadsheets — legal, fuel retail, wholesale distribution, regulated storefronts.{" "}
  ...
  return (
    <section id="work" style={{ position:"relative", padding:"144px clamp(24px,5vw,96px)" }}>
      <div style={{ margin:"0 auto", maxWidth:1200 }}>
        <window.SectionHeader num="01" eyebrow="Selected Work" title="What we ship" topLine={false} />
        {/* Featured */}
                  ...
                  <div style={{ display:"flex", alignItems:"center" }}>
                    <div style={{ width:"100%", aspectRatio:"4/3", borderRadius:8,
                      ...
                      <div style={{ textAlign:"center" }}>
                        <span style={{ fontFamily:"var(--font-mono)", fontSize:12, color:"rgba(255,42,193,.3)" }}>{featured.name.toLowerCase()}</span>
                        <span style={{ display:"block", marginTop:4, fontSize:10, color:"rgba(157,180,216,.25)" }}>{featured.category}</span>
                      </div>
                    </div>
                  </div>
                </div>
...
const FOUNDERS = [
  { first:"Stiles", last:"Dichter", role:"Co-founder · CEO",
    focus:"Sales · GTM · Ops", email:"stiles.dichter@gmail.com",
    bio:"CEO and co-founder. Stiles runs GTM, client relationships, and the commercial side of every product we ship. Owns the customer conversation end-to-end — from first call to renewal." },
  { first:"Joshua", last:"Langsam", role:"Co-founder · CTO",
    focus:"Engineering · AI · Research", email:"josh@roanco.law",
    bio:"CTO and co-founder. Josh ships the product. Greenfield work in Elixir/Phoenix, existing systems in TypeScript/Next.js, Claude + MCP for the intelligent bits. Four open-access papers on Zenodo." },
];
window.About = function About() {
  return (
    <section id="about" style={{ position:"relative", padding:"144px clamp(24px,5vw,96px)" }}>
      <div style={{ margin:"0 auto", maxWidth:1200 }}>
        <window.SectionHeader num="02" eyebrow="Founders" title="Two operators, one company." />
        <window.FadeIn delay={.1}>
          <p style={{ marginTop:24, maxWidth:760, fontSize:16, lineHeight:1.85, color:"var(--text-muted)" }}>
            Roan Co. is a Delaware C-Corp founded 50/50 by Stiles Dichter and Joshua Langsam.
            We build AI products and ship client systems under one roof — the sales lens and the
            engineering lens, run by two people who talk every day.
          </p>
        </window.FadeIn>
          ...
          {FOUNDERS.map((f, i) => (
            <window.FadeIn key={f.last} delay={.15 + i * .1}>
              <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)",
                borderRadius:8, padding:"40px 36px", height:"100%", display:"flex", flexDirection:"column" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
                  <div style={{ height:1, width:24, background:"rgba(255,42,193,.5)" }} />
                  <span style={{ fontFamily:"var(--font-mono)", fontSize:10,
                    letterSpacing:".28em", textTransform:"uppercase", color:"var(--accent)" }}>{f.role}</span>
                </div>
                <h3 style={{ fontFamily:"var(--font-serif)", fontWeight:400, fontSize:40,
                  margin:0, lineHeight:1, letterSpacing:"-.015em", color:"var(--cream)" }}>
                ...
                <div style={{ marginTop:"auto", paddingTop:28, borderTop:"1px solid var(--border)",
                  display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <span style={{ fontFamily:"var(--font-mono)", fontSize:11,
                    letterSpacing:".2em", textTransform:"uppercase", color:"rgba(157,180,216,.5)" }}>{f.focus}</span>
                  <a href={`mailto:${f.email}`} style={{ fontFamily:"var(--font-mono)", fontSize:11,
                    letterSpacing:".12em", color:"var(--cream)", textDecoration:"none" }}
                    onMouseEnter={e=>{ e.currentTarget.style.color="var(--accent)"; }}
                    onMouseLeave={e=>{ e.currentTarget.style.color="var(--cream)"; }}>{f.email} →</a>
                </div>
              </div>
            </window.FadeIn>
          ))}
