...
const links = [
  ...
  { num: "03", label: "Research", href: "#research" },
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-[1200px] flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center gap-10">
            {links.map((link) => (
              ...
              >
                <span className="font-mono text-accent/30 mr-1.5 group-hover:text-accent/60 transition-colors duration-300">
                  {link.num}
                </span>
                {link.label}
              </a>
          ...
          <a
            ...
            className="hidden md:block text-xs tracking-[0.12em] uppercase text-accent hover:text-text transition-colors duration-300"
          >
            Get in touch
          </a>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className="fixed inset-0 z-40 bg-bg flex flex-col justify-center items-center gap-10"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            {links.map((link, i) => (
              ...
              >
                <span className="font-mono text-sm text-accent/40">
                  {link.num}
                </span>
                <span className="font-serif text-4xl text-text hover:text-accent transition-colors">
                  {link.label}
                </span>
              </motion.a>
            ))}
